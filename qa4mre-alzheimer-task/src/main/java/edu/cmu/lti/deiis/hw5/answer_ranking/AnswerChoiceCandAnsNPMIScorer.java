...
public class AnswerChoiceCandAnsNPMIScorer extends JCasAnnotator_ImplBase {
  ...
  public double scoreCoOccurInSameDoc(String question, Answer choice) throws Exception {
    ...
    for (int i = 0; i < choiceNounPhrases.size(); i++) {
      ...
      double Z = 0;// define a big number for representing all the docs.

      /**
       * Here we define Z to be the minimal number that can be defined.
       */
      Z = nHits1 + nHits2 - combinedHits;
      // Z = Z*Z;
      score += myLogMI(combinedHits, nHits1, nHits2, Z);

      if (nHits1 != 0) {
        scoreCond += (double) combinedHits / nHits1;
      }

      boolean NMI = false;
      if (NMI)
        score2 += myLogScore2(combinedHits, Z);
    }
    if (choiceNounPhrases.size() > 0) {
      // score=score/choiceNounPhrases.size();
    }
    return 0.5*(score / score2) + 0.5 * scoreCond;
  }
//NPMI
  public double myLog(long combined, long nHits1, long nHits2, double Z) {
    if (combined == 0 || nHits1 == 0 || nHits2 == 0 || Z == 0) {
      return 0;
    }
    double logValue = Math.log(combined) - Math.log(nHits1) - Math.log(nHits2) + Math.log(Z);
    double deno = Math.log(Z) - Math.log(combined);
    logValue = logValue / deno;
    return logValue;
  }
// MI
  public double myLogMI(long combined, long nHits1, long nHits2, double Z) {
    if (combined == 0 || nHits1 == 0 || nHits2 == 0 || Z == 0) {
      return 0;
    }
    double logValue = Math.log(combined) - Math.log(nHits1) - Math.log(nHits2) + Math.log(Z);
    double deno = Math.log(combined) / Math.log(Z);
    logValue = logValue * deno;
    return logValue;
  }
// numeritor for NMI
  public double myLogScore2(long combined, double Z) {
    if (combined == 0 || Z == 0) {
      return 0;
    }
    double logValue = Math.log(combined) - Math.log(Z);
    double deno = Math.log(combined) / Math.log(Z);
    logValue = logValue * deno;
    return logValue;
  }
