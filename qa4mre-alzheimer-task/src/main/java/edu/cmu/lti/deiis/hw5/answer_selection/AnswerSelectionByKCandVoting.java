...
public class AnswerSelectionByKCandVoting extends JCasAnnotator_ImplBase {
  ...
  public double getBestScore(ArrayList<CandidateSentence> sents) {
   ...
   double score = getBestScore(sents);
   if (score > bestScore) {
   bestAnswer = answer.getText();
   bestScore = score;
   }
   }
   if (showInfo) {
   System.out.println("Correct Choice: " + "\t" + correctAnswer);
   System.out.println("Best Choice: " + "\t" + bestAnswer);
   }
